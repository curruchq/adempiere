...
//	private FlowLayout northLayout = new FlowLayout();
	...
	private Label kStatusLabel = new Label();
	private Label kStatusField = new Label();
	...
	private Label kCCVCLabel = new Label();
	private Textbox kCCVCField = new Textbox();
	private Label kAccountNameLabel = new Label();
	private Textbox kAccountNameField = new Textbox();
	private Label kOrigTxnIDLabel = new Label();
	private Textbox kOrigTxnIDField = new Textbox();
	private Checkbox kRefundCheckBox = new Checkbox();
	private Label kRefundLabel = new Label();
...
//		kNumberField.setHeight("21em");
		kCCVCField.setWidth("40pt"); // JH
//		kCCVCField.setHeight("21em"); // JH
		kExpField.setWidth("40pt");
//		kExpField.setHeight("21em");
		kAccountNameField.setWidth("120pt"); // JH
//		kAccountNameField.setHeight("21em"); // JH
		kApprovalField.setWidth("120pt");
//		kApprovalField.setHeight("21em");
		kOrigTxnIDField.setWidth("120pt"); // JH
//		kOrigTxnIDField.setHeight("21em"); // JH
		kRefundCheckBox.setSelected(false); // JH
		kOrigTxnIDField.setEnabled(false); // JH
...
//		kStatus.setText(" ");
		kStatusLabel.setText(Msg.translate(Env.getCtx(), "PaymentDocumentNo")); // JH
		kPanel.setId("kPanel");
		...
		row = rows.newRow();
		row.appendChild(kCCVCLabel.rightAlign());
		row.appendChild(kCCVCField);
		row.appendChild(new Space());
		row.appendChild(new Space());
		...
		row = rows.newRow();
		row.appendChild(kAccountNameLabel.rightAlign());
		row.appendChild(kAccountNameField);
		row.appendChild(new Space());
		row.appendChild(new Space());
		...
		row = rows.newRow();
		row.appendChild(kOrigTxnIDLabel.rightAlign());
		row.appendChild(kOrigTxnIDField);
		row.appendChild(new Space());
		row.appendChild(new Space());
		
		row = rows.newRow();
		row.appendChild(kRefundLabel.rightAlign());
		row.appendChild(kRefundCheckBox);
		row.appendChild(new Space());
		row.appendChild(new Space());
		
		row = rows.newRow();
//		row.setSpans("3,1");
		row.appendChild(kStatusLabel.rightAlign());
		row.appendChild(kStatusField);
		row.appendChild(new Space());
		row.appendChild(new Space());
...
//				kStatus.setText(m_mPayment.getR_PnRef()); 
				kStatusField.setText(m_mPayment.getR_PnRef()); // JH
				...
				kCCVCField.setReadonly(m_mPayment.isApproved()); // JH
				kAccountNameField.setReadonly(m_mPayment.isApproved()); // JH
				kOrigTxnIDField.setReadonly(m_mPayment.isApproved()); // JH
				kRefundCheckBox.setEnabled(!m_mPayment.isApproved()); // JH
		...
		// Handle display of status label and status field - JH
		String paymentRef = m_mPayment.getR_PnRef();
		if (paymentRef != null && paymentRef.length() > 0)
		{
			kStatusLabel.setVisible(true);
			kStatusField.setVisible(true);
			kStatusField.setText(paymentRef);
		}
		else
		{
			kStatusLabel.setVisible(false);
			kStatusField.setVisible(false);
			kStatusField.setText(" ");
		}
	...
	public void onEvent(Event e)
	{
		...
		//	Refund Checkbox - JH
		else if (e.getTarget() == kRefundCheckBox)
		{
			kOrigTxnIDField.setEnabled(kRefundCheckBox.isSelected());
			// If not selected we dont need TxnId for refund
