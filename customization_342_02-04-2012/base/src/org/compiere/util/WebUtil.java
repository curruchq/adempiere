...
public final class WebUtil
{
	...
	{
		return sendEMail(request, to, null, msgType, parameter);
	}
	
	/**
	 * 	Send EMail (with from address)
	 *	@param request request
	 *	@param to web user
	 ...
	 * 	@return mail EMail.SENT_OK or error message 
	 */
	public static String sendEMail (HttpServletRequest request, WebUser to, String from,
		String msgType, Object[] parameter)
	{
		WebSessionCtx wsc = WebSessionCtx.get(request);
		...
		{
			// do nothing as this is common case
		}
		// Simple validation check for from address
		else if (!from.contains("@"))
		{
			log.info("Invalid from address -> " + from + ", to -> " + to.getEmail() + ", MsgType -> " + msgType);
		}
		else
		{
			InternetAddress origFrom = email.getFrom();
			email.setFrom(from);
			if (!email.isValid(true) && origFrom != null)
			{
				email.setFrom(origFrom.getAddress());
			}
		}
	...
	{
		// Load web store object to get no reply email from - JH
		WebSessionCtx wsc = WebSessionCtx.get(request);
		MStore wStore = wsc.wstore;
		
		String msg = sendEMail(request, wu, wStore.getNoReplyEMail(),
	...
	/**
	 * Updates a web user's invoice schedule
	 * @param ctx
	 * @param wu
	 */
	public static void updateInvoiceSchedule(Properties ctx, WebUser wu)
	{
		if (wu == null)
		{
			log.fine("Could not update Invoice Schedule, WebUser = NULL");
			return;
		}
		...
		{
			int C_InvoiceSchedule_ID = invoiceSchedule.get_ID();
			MBPartner bp = MBPartner.get(ctx, wu.getC_BPartner_ID());
			if (C_InvoiceSchedule_ID > 0 && bp != null)
			{
				bp.setC_InvoiceSchedule_ID(C_InvoiceSchedule_ID);
				if (!bp.save())
				{
					log.warning("BPartner's save failed when updating Invoice Schedule ID, BPartner_ID=" + wu.getC_BPartner_ID() + ", C_InvoiceSchedule_ID=" + C_InvoiceSchedule_ID);
				}
			}
			else
			{
				log.warning("Could not update BPartner's Invoice Schedule, BPartner_ID=" + wu.getC_BPartner_ID() + ", C_InvoiceSchedule_ID=" + C_InvoiceSchedule_ID);
			}
		}
		else
		{
			log.warning("Could not find a MInvoiceSchedule for today, DAY_OF_MONTH=" + dayOfMonth);
		}
