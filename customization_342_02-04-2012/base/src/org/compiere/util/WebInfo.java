...
public class WebInfo
{
	...
	public ArrayList<MPayment> getPayments()
	{
		...
		// If BP doesn't have a validated credit card then hide the payment 
		// which was used for the credit card validation process - JH
		if (m_wu != null)
		{
			MBPBankAccount ba = m_wu.getBankAccount(true, false);
			if (ba != null && !ba.isBP_CC_Validated() && ba.getBP_CC_ValidationAmount() != null)
			{
				MPayment paymentToRemove = null;
				for (MPayment payment : list)
				{
					if (payment.getPayAmt() != null &&
						payment.getPayAmt().compareTo(ba.getBP_CC_ValidationAmount()) == 0)
					{
