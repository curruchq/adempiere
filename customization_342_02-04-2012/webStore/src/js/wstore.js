...
function popUpDesc(link, windowname)
{
	if (! window.focus)
		return true;
	var href;
	if (typeof(link) == 'string')
	   ...
	   href=link.href;
	newwin=window.open(href, windowname, 'toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable=1,width=500,height=450');
	newwin.focus();
	return false;
}

function popUpSearch(link, windowname)
{
	if (! window.focus)
		return true;
	var href;
	if (typeof(link) == 'string')
	   ...
	   href=link.href;
	newwin=window.open(href, windowname, 'toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable=1,width=540,height=200');
	newwin.focus();
	return false;
}
/**
...
AJAX.showLoading=function()
{
	...
	if (loadingElement != null && loadingElement)
	{
...
//	var height = Math.max(menu.offsetHeight, Math.max(content.offsetHeight, vendor.offsetHeight));
	var height = Math.max(menu.offsetHeight, content.offsetHeight);
