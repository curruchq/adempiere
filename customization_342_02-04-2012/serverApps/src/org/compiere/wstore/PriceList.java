...
public class PriceList
{
		...
		String searchString, String productCategory, boolean allRecords)
	{
		...
		if (retValue == null)
		{
			retValue = new PriceList (ctx, AD_Client_ID, M_PriceList_ID, 
				search, M_Product_Category_ID, allRecords, false);
			if (search == null && M_Product_Category_ID == 0 && allRecords)
	...
	/**
	 * Get PriceList (ignores isSelfService value) - JH
	 * 
	 * @param ctx
	 * @param AD_Client_ID client
	 * @param M_PriceList_ID price list
	 * @param M_Product_Category_ID product category
	 * @return Price list
	 */
	public static PriceList get (Properties ctx, int AD_Client_ID, int M_PriceList_ID, Integer M_Product_Category_ID)
	{
		PriceList retValue = null;
		String key = String.valueOf(AD_Client_ID) + "_" + M_PriceList_ID + "_" + M_Product_Category_ID;
		
		// Search PriceList Cache
		//retValue = (PriceList)s_cache.get(key);
		
		// Create new
		if (retValue == null)
		{
			retValue = new PriceList (ctx, AD_Client_ID, M_PriceList_ID, null, M_Product_Category_ID, true, true);
			s_cache.put(key, retValue);
		}
	 ...
	 *	@param allRecords all if no query
	 *  @param ignoreSelfService true to ignore isSelfService value
	 */
	private PriceList (Properties ctx, int AD_Client_ID, int M_PriceList_ID, 
		String searchString, int M_Product_Category_ID, boolean allRecords, boolean ignoreSelfService)
	{
		...
		loadProducts (searchString, M_Product_Category_ID, allRecords, ignoreSelfService);
	}	//	PriceList

	/**	Attribute Name - also in JSPs		*/
	public static final String		NAME = "priceList";
	public static final String		NAME_CATEGORY = "priceList_Category";
	/**	Logging						*/
	 ...
	 *	@param ignoreSelfService true to ignore isSelfService value
	 */
	private void loadProducts (String searchString, int M_Product_Category_ID, boolean allRecords, boolean ignoreSelfService)
	{
		//	Set Search String
		...
		String sql = "SELECT p.M_Product_ID, p.Value, p.Name, p.Description, "	//	1..4
			...
			+ " AND pp.PriceStd > 0 ";
		if (!ignoreSelfService)
			sql += " AND p.IsSelfService='Y' ";
		if (searchString != null)
