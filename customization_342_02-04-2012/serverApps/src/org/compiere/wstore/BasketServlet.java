...
public class BasketServlet extends HttpServlet
{
	...
	/** Description action (JH)			*/
	public static final String		A_DESCRIPTION = "desc";
	/** Product attribute (JH)			*/
		...
		throws ServletException, IOException
	{
		...
		// Handle action - JH
		String action = WebUtil.getParameter(request, P_ACTION);
		if (action == null || action.length() < 1)
		{
			// Do nothing
		}
		// Load a products description
		else if (action.equalsIgnoreCase(A_DESCRIPTION))
		{
			// 
			int M_Product_ID = WebUtil.getParameterAsInt(request, P_ID);
			...
			// TODO: Remove hard coded jsp value
			forward(request, response, "productDescription.jsp");
			return;
		}
