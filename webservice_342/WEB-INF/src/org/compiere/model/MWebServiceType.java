...
public class MWebServiceType extends X_WS_WebServiceType
{
	...
	/** Roles		*/
	private X_WS_WebServiceTypeAccess[] m_roles = null;
	...
	public static MWebServiceType get(Properties ctx, int WS_WebService_ID, int WS_WebServiceMethod_ID, String webServiceTypeValue, String trxName)
	{
		...
		try
		{
			...
			pstmt.setInt(1, WS_WebService_ID);
			pstmt.setInt(2, WS_WebServiceMethod_ID);
			pstmt.setString(3, webServiceTypeValue); 
		...
		{
			s_log.log(Level.SEVERE, "Failed to get MWebServiceType[" + webServiceTypeValue + "]", ex);
		}
		finally
		{
			DB.close(rs, pstmt);
	...
	{
		return "MWebServiceType[" + getWS_WebServiceType_ID() + "-" + getValue() + "]";
	}
	
	/**
	 * Validates user's roles against WebServiceType's roles
	 * 
	 * @param userRoles
	...
	public boolean validRole(KeyNamePair[] userRoles)
	{
		...
		for (X_WS_WebServiceTypeAccess accessRole : accessRoles)
		{
			...
			{
				if (role.getKey() == accessRole.getAD_Role_ID())
				{	
					return true;
				}
			}
	...
	public X_WS_WebServiceTypeAccess[] getAccessRoles(boolean requery)
	{
		if (m_roles != null && !requery)
			return m_roles;
		
		String sql = "SELECT * FROM " + X_WS_WebServiceTypeAccess.Table_Name + 
					" WHERE WS_WebServiceType_ID=? AND IsActive='Y'";
		
		ArrayList<X_WS_WebServiceTypeAccess> accessRoles = new ArrayList<X_WS_WebServiceTypeAccess>();
		...
		try
		{
			...
			rs = pstmt.executeQuery();
			while (rs.next())
				accessRoles.add(new X_WS_WebServiceTypeAccess(getCtx(), rs, get_TrxName()));
		}
		catch (Exception ex)
		{
			log.log(Level.SEVERE, "Failed to load WS_WebServiceTypeAccess", ex);
		}
		finally
		{
			DB.close(rs, pstmt);
		...
		// Convert to array
		m_roles = new X_WS_WebServiceTypeAccess[accessRoles.size()];
		accessRoles.toArray(m_roles);
