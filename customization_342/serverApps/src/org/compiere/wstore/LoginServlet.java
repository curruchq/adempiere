...
public class LoginServlet extends HttpServlet
{
	...
	/** Default JSP (JH)			*/
	public static final String 	JSP_DEFAULT = "login.jsp";
	/** Signup JSP (JH)				*/
...
//		session.setAttribute (WebUser.NAME, wu);
		...
		if (thisLogin.getMode()==null) 
		{
			String s = WebUtil.getParameter (request, thisLogin.getP_Action());
			thisLogin.setMode(s);
		}
		
		if ("Submit".equals(thisLogin.getMode()))
		{
			WebUser tmpWU = wu;
			if (tmpWU == null || !tmpWU.isLoggedIn())
			{
				String email = WebUtil.getParameter (request, thisLogin.getP_EMail());
				tmpWU = WebUser.get (ctx, email, null, false); //	load w/o password check direct
			}
			
			// If new user validate captcha before processing action
			if (tmpWU.getAD_User_ID() == 0)
			{
				if (!CaptchaServlet.validateCaptcha(request))
				{
					processAction = false;
					url = "/signup.jsp";
				}
		...
		if (processAction)
		{
			...
			// Check contract
			if (wu != null && wu.isLoggedIn() && !wu.validContract())
			{
				url = ContractServlet.JSP_DEFAULT + "?" + ContractServlet.REFERER_NAME + "=" + url;
			}
		}
