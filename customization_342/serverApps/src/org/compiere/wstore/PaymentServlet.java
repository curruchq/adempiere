...
public class PaymentServlet  extends HttpServlet
{
	...
	//	WEnv.dump(request);
		...
		if (wu == null || p == null)
			...
			url = processPayment(request, response, ctx, p, wu, wo);
	 ...
	 */
	private String processPayment(HttpServletRequest request, HttpServletResponse response, Properties ctx, 
		MPayment payment, WebUser wu, WebOrder wo)
	{
		String retValue = "/paymentInfo.jsp";
		...
		if (ok)
		{
			...
			// check for Credit card validation payment flag
			boolean ccvPayment = session.getAttribute(CreditCardValidationServlet.ATTR_CCV_PAYMENT) != null;
			
			if (ccvPayment)
			{
				session.removeAttribute(ATTR_PAYMENT);
				session.removeAttribute(CreditCardValidationServlet.ATTR_CCV_PAYMENT);
				
				String msg = CreditCardValidationServlet.chargeAccount(this, request, response, payment);
				if (msg != null)
					request.setAttribute(CreditCardValidationServlet.INFO_MSG, msg);
				
				retValue = "/" + CreditCardValidationServlet.DEFAULT_JSP;
			}
			else
			{
				...
				if (ok)
				{
					...
					if (wo != null)
					{				
						// check if customers credit card is validated
						boolean validated = false;
						MBPBankAccount ba = wu.getBankAccount();
						if (ba != null)
							validated = ba.isBP_CC_Validated();
						else
							log.severe("Payment has been processed without a Bank Account?! Outbound calling for DIDs in this order (" + wo.getC_Order_ID() + ") has been barred.");				
						
						String validationConfirm = WebUtil.getParameter(request, "validationConfirm");
						...
						if (DIDController.getDIDNumbers(wo.getOrder(), false).size() > 0)							
							request.setAttribute("showDIDMsg", "true");
						
						// user ticked 'charge account'
						if (!validated && validationConfirm != null)
						{
							// charge customers credit card
							String errorMsg = CreditCardValidationServlet.chargeAccount(this, request, response);
							if (errorMsg != null)
							{
								log.warning("Could not charge customers account - Error: " + errorMsg);
							}
						}						
						// user did not tick 'charge account'
						else if (!validated  && validationConfirm == null)
						{
							CreditCardValidationServlet.sendValidationRequestEmail(request, wu);
						}
					}
					
					retValue = "/confirm.jsp";
				}
				else
				{
					// no web order when making a direct payment
