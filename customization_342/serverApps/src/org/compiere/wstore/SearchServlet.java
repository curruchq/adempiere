...
public class SearchServlet extends HttpServlet
{
    ...
    /** Request parameter for mode (JH)							*/
    public static final String P_MODE = "mode";
    
    /** Request parameter for set input (JH)					*/
    public static final String P_SET_INPUT = "setInput";
    
    /** Request parameter value for business partner mode (JH)	*/
    public static final String M_BUSINESS_PARTNER = "bp";
    
    /** Request parameter value for country id (JH)				*/
    public static final String M_COUNTRY_ID = "countryId";
    
    /** Request parameter value for user mode (JH) 				*/
        ...
        throws ServletException, IOException
    {
        ...
        // Handle mode - JH
        String mode = WebUtil.getParameter(request, P_MODE);
        if (mode != null && mode.length() > 0)
        {
        	handleSearch(request, response, mode);
        	return;
        }
...
//        MLocation loc = new MLocation (ctx, 0, null);
    ...
    private void handleSearch(HttpServletRequest request, HttpServletResponse response, String mode)
		throws ServletException, IOException
	{
	    Properties ctx = JSPEnv.getCtx(request);
	    ...
	    if (mode.equalsIgnoreCase(M_BUSINESS_PARTNER))
	    {
	    	if (name == null || name.length() < 1)
	    		name = " ";
	    	ArrayList<MBPartner> businessPartners = MBPartnerEx.getByName(ctx, name);
	    	if (businessPartners.size() > 0)
	    		request.setAttribute("result", businessPartners);
	    }
	    else if (mode.equalsIgnoreCase(M_COUNTRY_ID))
	    {
	    	...
	    	if (name == null || name.length() < 1)
	    	{
	    		request.setAttribute("result", countries);
	    	}
	    	else
	    	{
	    ...
	    else if (mode.equalsIgnoreCase(M_USER))
	    {
	    	if (name == null)
	    		name = "";
