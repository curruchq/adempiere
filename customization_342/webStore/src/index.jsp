...
<div id="main">
	...
	<div id="content">
      <form action="productServlet" method="post" enctype="application/x-www-form-urlencoded" name="search" id="search">
		  ...
		  <c:if test='${priceList.notAllPrices and !priceList.noLines}'>
        ...
        <table class="contentTable">
          <tr> 
          	<c:forEach items='${priceList.prices}' var='product' varStatus='status'> 
          		<c:if test='${empty COL_SPAN and not empty product.imageURL}'>
            		<c:set var="COL_SPAN" value="2"/>
           		</c:if>
          	</c:forEach>
          	<c:if test='${empty COL_SPAN}'>
            	<c:set var="COL_SPAN" value="1"/>
            </c:if>
            <th colspan="<c:out value='${COL_SPAN}'/>" align="left">Product</th>
            <th>Description</th>
          ...
          <c:forEach items='${priceList.prices}' var='product' varStatus='status'> 
        	...
        	<td class="<c:out value='${rowClass}' />">
        		...
        		<c:if test='${empty product.imageURL }'>
        			<c:out value='${ctx.webParam3}' default=' '/>
        		</c:if>
        	</td>
