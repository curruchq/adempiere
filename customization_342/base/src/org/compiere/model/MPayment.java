	...
	implements DocAction, ProcessCall
{
	...
	public boolean saveToBP_BankAccount (MBPBankAccount ba)
	{
		...
		// Check if credit card details are being changed, if so reset
		// credit card validation fields - JH
		String currentCCType = ba.getCreditCardType();
		String currentCCNumber = ba.getCreditCardNumber();
		String newCCType = getCreditCardType();
		String newCCNumber = getCreditCardNumber();
	
		if ((currentCCType == null || currentCCNumber == null || 
			currentCCType.length() < 1 || currentCCNumber.length() < 1)
			||
			(!currentCCType.equalsIgnoreCase(newCCType) || 
			 !currentCCNumber.equalsIgnoreCase(newCCNumber)))
		{
		...
		// Never save CreditCardVV code as a security precaution - JH 
	 ...
	 *	@return Name
	 */
	public static String getCreditCardName(String CreditCardType)
	{
	...
	public String prepareIt()
	{
		...
		{
			m_processMsg = "Payment Allocate must not exists if the payment header has charge/invoice/order.";
			return DocAction.STATUS_Invalid;
		}
		...
		{
			m_processMsg = "Payment Amount must be equal to the sum of Allocate amounts.";
			return DocAction.STATUS_Invalid;
		}
