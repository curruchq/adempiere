...
public class MDunningRun extends X_C_DunningRun
{
	 ...
	 *	@return entry
	 */
	public MDunningRunEntry getEntryByBPLocation (int C_BPartner_ID, int C_Currency_ID, int SalesRep_ID, int C_BPartner_Location_ID)
	{
		// TODO: Related BP
		int C_BPartnerRelated_ID = C_BPartner_ID;
		//
		...
		{
			MDunningRunEntry entry = m_entries[i];
			if (entry.getC_BPartner_ID() == C_BPartnerRelated_ID && entry.getC_BPartner_Location_ID() == C_BPartner_Location_ID)
				return entry;
		}
		//	New Entry
		MDunningRunEntry entry = new MDunningRunEntry (this);
		...
		if (entry.getSalesRep_ID() == 0)
			entry.setSalesRep_ID (SalesRep_ID);
		entry.setC_Currency_ID (C_Currency_ID);
		entry.setC_BPartner_Location_ID(C_BPartner_Location_ID);
		//
		m_entries = null;
		return entry;
	}	//	getEntry
