...
public class CalloutInvoice extends CalloutEngine
{
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		MProductPricing pp = new MProductPricing (M_Product_ID.intValue(), C_BPartner_ID, Qty, IsSOTrx, C_Invoice_ID);
		//
	...
	public String tax (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		int C_Tax_ID = getTax(ctx, AD_Org_ID);
	 ...
	 * 	@return C_Tax_ID
	 */
	private int getTax (Properties ctx, int AD_Org_ID)
	{
		int C_Tax_ID = 0;
		String sql = "SELECT t.C_Tax_ID "
			+ "FROM C_Tax t "
			+ "WHERE t.AD_Org_ID=? AND t.IsDefault='Y'"
			;
		boolean found = false;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
