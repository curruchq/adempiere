...
//			WebUtil.createForwardPage(m_response, "Logout", "http://" + m_request.getServerName() + "/", 2);
			...
			// Forward/redirect without using javascript - JH 
			if (!m_response.isCommitted())
			{
				String serverContext = m_ctx.getProperty(WebSessionCtx.CTX_SERVER_CONTEXT);
				m_response.sendRedirect(m_response.encodeRedirectURL("http://" + serverContext));
			}
		...
		else if ("Submit".equals(m_mode))
		{
			...
			else	//	new
			{
				...
				{
					// Update new user's invoice schedule 
					WebUtil.updateInvoiceSchedule(m_ctx, m_wu);
				
					// Force usual login process (which calls contract/t&c code) - JH
...
//					else
        ...
        else if ("bankaccountach".equals(m_mode))
        {
    		...
    		MBPBankAccount thisBPBankAccount = m_wu.getBankAccount (true, true);
