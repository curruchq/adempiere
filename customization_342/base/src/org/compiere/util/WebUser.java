...
import com.conversant.model.WebUserContract;
...
public class WebUser
{
	...
	/** Number of days contract / terms & conditions agreement are valid for  */
	...
	public boolean save()
	{
		...
		try
		{
			...
			// Commented out so that BP search key (Value) is set to
			// a sequential numeric value as opposed to an email address - JH
...
//				m_bp.setValue(m_bpc.getEMail());
	...
	{
		return m_bpc.getFirstName();
	}
	
	public String getLastName()
	{
		return m_bpc.getLastName();
	}
	...
	public String getBP_SearchKey ()
	{
		return m_bp.getValue();
	}
	...
	{
		return getBankAccount(false, true);
	}
	
	/**************************************************************************
	 * 	Get BP Bank Account (or create it)
	 *  @param requery Requery data
	 *  @param create new if not found
	 *	@return Bank Account
	 */
	public MBPBankAccount getBankAccount(boolean requery, boolean createNew)
	{
	...
	/************************************************
	 * 	Methods added below by Josh Hill 29/10/2008	*
	 ************************************************/
	...
	private MAttachment getAttachment()
	{
		...
		if (m_bpc != null)
		{
			int AD_Table_ID = m_bpc.get_Table_ID();
			int Record_ID = m_bpc.get_ID();
			
			// load existing
			attachment = MAttachment.get(m_ctx, AD_Table_ID, Record_ID);
			
			// create new
			if (attachment == null)
			{
				attachment = new MAttachment(m_ctx, AD_Table_ID, Record_ID, null);
				attachment.save();
			}
		}
	 ...
	 */
	private boolean addAttachmentEntry(String text, File file)
	{
		MAttachment attachment = getAttachment();
		
		if (attachment != null)
		{
			attachment.addTextMsg(text);
			
			boolean fileExists = false;
			for (MAttachmentEntry entry : attachment.getEntries())
			{
				if (entry.getName().equalsIgnoreCase(file.getName()))
				{
					fileExists = true;
					break;
				}
			...
			if (!fileExists)
			{
				if (attachment.addEntry(file))
					return attachment.save();
			}
			// dont need to add file as already exists
			else
			{
				return attachment.save();
			}
	...
	/**
	 * Add contract attachment against user
	 * @param name
	 * @param ip
	 * @param path
	 * @param filename
	 ...
	 */
	public boolean addContractAttachment(String name, String ip, String path, String filename)
	{
		Date now = new Date();		
		SimpleDateFormat sdf = new SimpleDateFormat();
		
		String text = "[Date=" + sdf.format(now) + ",Name=" + name + ",IP=" + ip + ",Filename=" + filename + "]";	
		
		File contract = new File(path + File.separator + filename);
		if (contract.exists())
			return addAttachmentEntry(text, contract);
		else
			log.warning("Couldn't find file " + contract.getAbsolutePath());
		
		return false;
	}	//  addContractAttachment
	
	/**
	 * Gets contracts (terms and conditions agreements) for this Web User.
	 * @return array list of web user contracts
	 */
	public ArrayList<WebUserContract> getContracts()
	{
		ArrayList<WebUserContract> contracts = new ArrayList<WebUserContract>();
		
		MAttachment attachment = getAttachment();		
		if (attachment != null)
		{
			String text = attachment.getTextMsg();
			if (text != null)
			{
				while (text.length() > 0)
				{
					...
					for (int i=startPos; i<text.length(); i++)
					{
						...
						contract.append(text.charAt(i));
					}					
					
					// remove from text string
					text = text.substring(endPos + 1);
					
					// create webUserContract object
					boolean error = false;
					WebUserContract webUserContract = new WebUserContract();
					StringTokenizer st = new StringTokenizer(contract.toString(), ",");
					while (st.hasMoreTokens())
					{
						...
						{
							webUserContract.setName(value);
						}
						else if (key.equalsIgnoreCase("IP"))
						{
							webUserContract.setIp(value);
						}
						else if (key.equalsIgnoreCase("Filename"))
						{
							webUserContract.setFilename(value);
						}						
					}
					
					if (!error)
						contracts.add(webUserContract);
				}							
			}
		}
	...
	/**
	 * Checks for valid contract 
	 * @return true if valid contract found
	 */
	public boolean validContract()
	{
		Calendar cal = GregorianCalendar.getInstance();
		cal.setTimeInMillis(System.currentTimeMillis());
		cal.add(Calendar.DAY_OF_MONTH, -VALID_DAYS);
		Date from = new Date(cal.getTimeInMillis());
		
		return validContract(from);
	}	//  validContract
	
	/**
	 * Checks for valid contract from the date given
	 * @param from 
	 * @return true if valid contract found
	 */
	public boolean validContract(Date from)
	{
		ArrayList<WebUserContract> contracts = getContracts();
		for (WebUserContract contract : contracts)
		{
			// if contract exists where date is after or equal to from then return true
			Date contractDate = contract.getDate();
			if (contractDate != null && contractDate.compareTo(from) >= 0)
			{
				return true;
			}
		}
	...
	/**
	 * Get BP's bank account (doesn't create new bank account) used by credit card validation JSP
	 * @return Business Partner's bank account or null
	 */
	public MBPBankAccount getBPBankAccount()
	{
		return getBankAccount(true, false);
	}	//  getBPBankAccount
	
	/**
	 * Gets credit card description consisting of card type and masked card number
	 * @return Masked credit card description or "-"
	 */
	public String getCreditCardDescription()
	{
		MBPBankAccount ba = getBankAccount(true, false);
		if (ba != null)
		{
			// build masked credit card number
			String ccNum = ba.getCreditCardNumber();
			StringBuilder maskedCCNum = new StringBuilder();
			for (int i=0;i<ccNum.length()-4;i++)
			{
				maskedCCNum.append("x");
			}
			maskedCCNum.append(" " + ccNum.substring(ccNum.length()-4, ccNum.length()));

			// build description string
			StringBuilder desc = new StringBuilder();		
			desc.append(MPayment.getCreditCardName(ba.getCreditCardType()));
			desc.append(" ");
			desc.append(maskedCCNum);
