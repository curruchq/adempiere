...
/**
 *  Inventory Valuation.
 *  Process to fill T_InventoryValueAU
 *
 *  @author     Lavanya
 *  @version    $Id: InventoryValueAU.java,v 1.2 2017/06/07 00:51:02$
 */
public class InventoryValueAU extends SvrProcess
{
	/** Price List Used         */
	...
	protected String doIt() throws Exception
	{
		...
		MAcctSchema as =new MAcctSchema(getCtx(),p_C_AcctSchema_ID,null);
		
		//  Delete (just to be sure)
		StringBuffer sql = new StringBuffer ("DELETE T_InventoryValueAU WHERE AD_PInstance_ID=");
		sql.append(getAD_PInstance_ID());
		int no = DB.executeUpdate(sql.toString(), get_TrxName());

		//	Insert Standard Costs
		sql = new StringBuffer ("INSERT INTO T_InventoryValueAU "
			+ "(AD_PInstance_ID, M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID,"
			+ " AD_Client_ID, AD_Org_ID, CostStandard,C_AcctSchema_Id)  "
			+ "SELECT ").append(getAD_PInstance_ID())
			.append(", w.M_Warehouse_ID, c.M_Product_ID, c.M_AttributeSetInstance_ID,"
			+ " w.AD_Client_ID, w.AD_Org_ID, c.CurrentCostPrice , ").append(p_C_AcctSchema_ID)
			.append(" FROM M_Warehouse w"
			+ " INNER JOIN AD_Client ci ON (w.AD_Client_ID=ci.AD_Client_ID)"
			//+ " INNER JOIN C_AcctSchema acs ON (ci.C_AcctSchema1_ID=acs.C_AcctSchema_ID)"
			+ " INNER JOIN M_Cost c ON (ci.AD_Client_ID=c.AD_Client_ID AND c.AD_Org_ID IN (0, w.AD_Org_ID))"
			+ " INNER JOIN C_AcctSchema acs ON (c.C_AcctSchema_ID=acs.C_AcctSchema_ID AND acs.M_CostType_ID=c.M_CostType_ID )"
			...
			.append(" AND c.C_AcctSchema_ID = ").append(p_C_AcctSchema_ID);
		...
		if (p_M_CostElement_ID != 0)
		{
			sql = new StringBuffer ("INSERT INTO T_InventoryValueAU "
				+ "(AD_PInstance_ID, M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID,"
				+ " AD_Client_ID, AD_Org_ID, CostStandard, Cost, M_CostElement_ID,C_AcctSchema_ID) "
				+ "SELECT ").append(getAD_PInstance_ID())
				.append(", w.M_Warehouse_ID, c.M_Product_ID, c.M_AttributeSetInstance_ID,"
				+ " w.AD_Client_ID, w.AD_Org_ID, 0, c.CurrentCostPrice, c.M_CostElement_ID , ").append(p_C_AcctSchema_ID)
				.append(" FROM M_Warehouse w"
				+ " INNER JOIN AD_Client ci ON (w.AD_Client_ID=ci.AD_Client_ID)"
			    //+ " INNER JOIN C_AcctSchema acs ON (ci.C_AcctSchema1_ID=acs.C_AcctSchema_ID)"
			    + " INNER JOIN M_Cost c ON (ci.AD_Client_ID=c.AD_Client_ID AND c.AD_Org_ID IN (0, w.AD_Org_ID))"
				...
				.append(" AND NOT EXISTS (SELECT * FROM T_InventoryValueAU iv "
			...
			sql = new StringBuffer ("UPDATE T_InventoryValueAU iv "
				+ "SET (Cost, M_CostElement_ID)="
					...
					+ " AND iv.M_Product_ID=c.M_Product_ID AND c.AD_Org_ID = w.AD_Org_ID " 
					+ " AND iv.M_AttributeSetInstance_ID=c.M_AttributeSetInstance_ID AND iv.C_AcctSchema_ID = "+p_C_AcctSchema_ID
				+ " ) WHERE EXISTS (SELECT * FROM T_InventoryValueAU ivv "
		...
		sql = new StringBuffer ("UPDATE T_InventoryValueAU SET ")
		...
		sql = new StringBuffer ("UPDATE T_InventoryValueAU iv SET QtyOnHand = "
		...
		sql = new StringBuffer("DELETE T_InventoryValueAU "
		...
		//  Update Prices
		no = DB.executeUpdate ("UPDATE T_InventoryValueAU iv "
			+ "SET PricePO = "
		...
		//  Update Values
		no = DB.executeUpdate("UPDATE T_InventoryValueAU SET "
